...
def cfg_writer(sample, isMC, outdir):
    ...
    if not isMC:
        ...
        elif sample.year == 'UL2016' or sample.year == "UL2016APV":
        ...
        if not ("DataHT" in sample.name and sample.year=='2018'):
    ...
    if submit:
        ...
        if ('Data' in sample.label):
            ...
            if year == '2016':# and sample.runP != 'H':
                ...
                if 'DataHT' not in sample.label:
                    presel += " && (HLT_IsoMu27 || HLT_Mu50 || HLT_Ele35_WPTight_Gsf || (HLT_Ele32_WPTight_Gsf_L1DoubleEG && (L1_SingleIsoEG30er2p1 || L1_SingleIsoEG32 || L1_SingleEG40)) || HLT_Photon200)"
                else:
                    presel += " && (HLT_PFHT250 || HLT_PFHT350)"# || HLT_PFHT370 || HLT_PFHT430 || HLT_PFHT510 || HLT_PFHT590 || HLT_PFHT680 || HLT_PFHT780 || HLT_PFHT890)"
            elif year == 'UL2018':
                if 'DataHT' not in sample.label:
        ...
        if "UL" in str(year):
            ...
            if isMC:
                modules = sampleFlag_mod + "MCweight_writer('" + sample.label + "'), " + met_hlt_mod + ", " + lep_mod + ", " + pu_mod + ", " + btag_mod + ", PrefireCorr_" + str(year) + "(), metCorrector(), fatJetCorrector(), " + muon_pt_corr + ", " + ht_producer + ", " + mht_producer # Put here all the modules you want to be runned by crab
            else:
