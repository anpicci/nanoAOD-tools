...
class FakeCalculator_manager:
    ...
    def Calc(self, sample, isData, onlybkg, met_cut, mt_lepMET_cut, trig, hEle, hMu, hTau):
        ...
        cut_ele_l = "abs(FakeLepton_pdgid)==11&&nLeps_LightLeptons>0&&nLeps_LightLeptons<=2&&Jet_numberSeparate>0&&abs(FakeLepton_eta)<2.4&&(abs(FakeLepton_eta)<1.4442||abs(FakeLepton_eta)>1.566)&&FakeLepton_pt>=0&&FakeLepton_jetRelIso>=0"
        cut_ele_t = "FakeLepton_jetRelIso<0.08&&FakeLepton_isTight"
        cut_mu_l =  "abs(FakeLepton_pdgid)==13&&nLeps_LightLeptons>0&&nLeps_LightLeptons<=2&&Jet_numberSeparate>0&&abs(FakeLepton_eta)<2.4&&FakeLepton_pt>=0&&FakeLepton_pfRelIso04>=0"
