...
def diet_AreThereAdditionalLooseTaus(taus, ele, mu):
    # ID_TAU_RECO_DEEPTAU_VSJET_LOOSE_ELE
    # ID_TAU_RECO_DEEPTAU_VSJET_LOOSE_MU
    # ID_TAU_RECO_DEEPTAU_VSJET_MEDIUM

    for tau in taus:
        if tau.idDeepTau2017v2p1VSjet   < 16:   #Medium WP
            continue
        if tau.idDeepTau2017v2p1VSe     < 8:    #Loose WP
            continue
        if tau.idDeepTau2017v2p1VSmu    < 2:    #Loose WP
            continue
        if deltaR(tau.eta, tau.phi, ele.eta, ele.phi) < 0.5 or deltaR(tau.eta, tau.phi, mu.eta, mu.phi) < 0.5: continue
...
def diet_SelectJet(jetCollection, GoodEle, GoodMu):
    ...
    GoodJet=jetCollection[0]
    #if the jet matches in dR one of the previously selected particles (e, tau), than it searches in the other jets
    if deltaR(GoodJet.eta, GoodJet.phi, GoodEle.eta, GoodEle.phi)<DR_OVERLAP_CONE_OTHER or deltaR(GoodJet.eta, GoodJet.phi, GoodMu.eta, GoodMu.phi)<DR_OVERLAP_CONE_OTHER: 
        jetCollection.remove(GoodJet)
        if len(jetCollection)==1:
             return False, False, False
        return diet_SelectJet(jetCollection, GoodEle, GoodMu)
    
    #searches for the best second jet
    secondJetIndex = diet_FindSecondJet(GoodJet, jetCollection, GoodEle, GoodMu)
    if secondJetIndex>0: return True, GoodJet, jetCollection[secondJetIndex]
    else:
        jetCollection.remove(GoodJet)
        if len(jetCollection)==1:
            return False, False, False
        else: return diet_SelectJet(jetCollection, GoodEle, GoodMu)
