...
def FindSecondJet(jet, jetCollection, GoodTau, GoodMu):
    for k in range(len(jetCollection)):
        ...
        if abs(jet.eta-jetCollection[k].eta)>DELTAETA_JJ_CUT:
            if deltaR(jet.eta, jet.phi, GoodTau.eta, GoodTau.phi)>DR_OVERLAP_CONE_TAU+DR_OVERLAP_CONE_OTHER and deltaR(jet.eta, jet.phi, GoodMu.eta, GoodMu.phi)>DR_OVERLAP_CONE_OTHER:
                return k
    return -1
...
def SelectJet(jetCollection, GoodTau, GoodMu):
    ...
    if secondJetIndex>0:
        print("isocone_tau:", DR_OVERLAP_CONE_TAU+DR_OVERLAP_CONE_OTHER, "isocone_lep", DR_OVERLAP_CONE_OTHER)
        print("jet1:\tdR_tau:", deltaR(GoodJet.eta, GoodJet.phi, GoodTau.eta, GoodTau.phi), "jet1:\tdR_lep:", deltaR(GoodJet.eta, GoodJet.phi, GoodMu.eta, GoodMu.phi))
        print("jet2:\tdR_tau:", deltaR(jetCollection[secondJetIndex].eta, jetCollection[secondJetIndex].phi, GoodTau.eta, GoodTau.phi), "jet2:\tdR_lep:", deltaR(jetCollection[secondJetIndex].eta, jetCollection[secondJetIndex].phi, GoodMu.eta, GoodMu.phi))
        return GoodJet, jetCollection[secondJetIndex]
    else:
        jetCollection.remove(GoodJet)
        if len(jetCollection)==1:
            return -999
