...
if opt.bveto:
    ...
    cut_dict = {'muon':"(abs(" + mpdgstr + "_pdgid)==13&&pass_lepton_selection==1&&pass_tau_selection==1&&pass_lepton_veto==1&&pass_jet_selection==1&&pass_b_veto==1&&MET_pt<=50.&&mT_lep_MET>50.)*(" + cut + ")", 
                'electron':"(abs(" + epdgstr + "_pdgid)==11&&pass_lepton_selection==1&&pass_tau_selection==1&&pass_lepton_veto==1&&pass_jet_selection==1&&pass_b_veto==1&&MET_pt<=50.&&mT_lep_MET>50.)*(" + cut + ")",
                'incl':"((abs(" + mpdgstr + "_pdgid)==13" + incl_logic + "abs(" + epdgstr + "_pdgid)==11)&&pass_lepton_selection==1&&pass_lepton_veto==1&&pass_jet_selection==1&&pass_b_veto==1&&pass_tau_veto==1&&MET_pt<=50.&&(mT_electron_MET>50.||mT_muon_MET>50.))*(" + cut + ")",
            }
    cut_tag = 'wjets_CR'
    if opt.cut != "1.":
        cut_tag = cut_tag+ '_AND_' + cutToTag(opt.cut)           
    ...
    cut_dict = {'muon':"(abs(" + mpdgstr + "_pdgid)==13&&pass_lepton_selection==1&&pass_tau_selection==1&&pass_lepton_veto==1&&pass_jet_selection==1&&pass_b_veto==1&&MET_pt<=50.&&mT_lep_MET<50.)*(" + cut + ")", 
                'electron':"(abs(" + epdgstr + "_pdgid)==11&&pass_lepton_selection==1&&pass_tau_selection==1&&pass_lepton_veto==1&&pass_b_veto==1&&pass_jet_selection==1&&MET_pt<=50.&&mT_lep_MET<50.)*(" + cut + ")",
                'incl':"((abs(" + mpdgstr + "_pdgid)==13" + incl_logic + "abs(" + epdgstr + "_pdgid)==11)&&pass_lepton_selection==1&&pass_tau_selection==1&&pass_b_veto==1&&pass_lepton_veto==1&&pass_jet_selection==1&&pass_tau_veto==1&&MET_pt<=50.&&(mT_electron_MET<50.&&mT_muon_MET<50.))*(" + cut + ")",
            }
    cut_tag = 'QCD_CR'
    if opt.cut != "1.":
        cut_tag = cut_tag+ '_AND_' + cutToTag(opt.cut)           
...
for year in years:
    ...
    for lep in leptons:
        ...
        if opt.wjets or opt.qcd:
            ...
            bin_zepp = array("f", [-1.5, -1., -0.75, -0.5, -0.25, 0., 0.25, 0.5, 0.75, 1., 1.5])#, 300.])#, 500.])
            nbin_zepp = len(bin_zepp)-1
        variables.append(variabile('event_Zeppenfeld_over_deltaEta_jj', 'event Zeppenfeld',  wzero+'*('+cutbase+')', 12, -1.5, 1.5))
        ...
        if opt.channel == "ltau":
            ...
            bin_taujetrelpt = array("f", [0.2, 0.6, 0.8, 0.9, 1.])
