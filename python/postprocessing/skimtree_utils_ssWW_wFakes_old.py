...
def trig_finder(HLT, yearr, samplename):
    vTrigEle = []
    vTrigMu = []
    vTrigHT = []
    ...
    if ("2016" in year):
        ...
        if HLT.IsoTkMu24:                                vTrigMu.append("IsoTkMu24")
        ...
        if HLT.Ele27_WPTight_Gsf:                        vTrigEle.append("Ele27_WPTight_Gsf")
        ...
        #if HLT.Photon175:                               vTrigEle.append("Photon175")
...
def SelectLepton(leptons, jet1 = None, jet2 = None):
    ...
    for lep in leptons:
        ...
        if lepflav == 11:
            ...
            IsTightID = lep.tightId
            IsLooseID = lep.looseId
    ...
    nTleps = len(Tleptons)
    nLnTleps = len(LnTleptons)
    sellep = None
    sellepstatus = -1

    #select leading tight/loose-not-tight lepton
    if not (nTleps == 0 and LnTleptons == 0):
        if nTleps > 0:
            sellep = Tleptons[0]
            sellepstatus = 1
        elif nLnTleps > 0:
...
def SFFakeRatio_ele_calc(pT, eta, year='2017', folder = "remote", wp = 'vsjet' + str(ID_TAU_RECO_DEEPTAU_VSJET_LOOSE_ELE)):
    ...
    if not year.startswith("UL"):
        ...
        histo=ROOT.TH2F(inFile.Get("FakeRatio_Electron"))
    FR=0
...
def SFFakeRatio_tau_calc(pT, eta, idl, year='2017', folder = "remote", wpp ='vsjet'):
    ...
    if not year.startswith("UL"):
        ...
        histo=ROOT.TH2F(inFile.Get("FakeRatio_Tau"))
...
def SFFakeRatio_mu_calc(pT, eta, year='2017', folder = "remote", wp = 'vsjet'+ str(ID_TAU_RECO_DEEPTAU_VSJET_LOOSE_MU)):
    ...
    if not year.startswith("UL"):
        ...
        histo=ROOT.TH2F(inFile.Get("FakeRatio_Muon"))
