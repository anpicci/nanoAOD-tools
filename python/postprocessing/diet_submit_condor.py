...
usage = 'python diet_submit_condor.py -d dataset_name -f destination_folder'
...
def sub_writer(sample, n, files, folder):
    ...
    f.write("transfer_input_files    = $(Proxy_path), samples/samples.py, diet_skimtree_utils.py, CutsAndValues_bu.py, FR_vsjet2_vsmuT.root, FR_vsjet4_vsmuT.root, ./data/leptonSF/Muon_RunBCDEF_SF_ID_2017.root, TauIDSFTool.py, Btag_eff.root, EFTOperator_dict.py, __init__.py, ./data\n")
    ...
    f.write("executable              = diet_tree_skimmer.py\n")
    f.write("arguments               = " + sample.label + " " + str(n) + " " + str(files) + " remote " + "\n")# + str(wopstring) + "\n")
    #f.write("input                   = input.txt\n")
    f.write("output                  = condor_" + opt.folder + "/output/"+ sample.label + "_part" + str(n) + ".out\n")
    f.write("error                   = condor_" + opt.folder + "/error/"+ sample.label +  "_part" + str(n) + ".err\n")
    f.write("log                     = condor_" + opt.folder + "/log/"+ sample.label + "_part" + str(n) + ".log\n")
...
for sample in samples:
    ...
    if(isMC):
        for i, files in enumerate(files_list):
            ...
            print("python diet_tree_skimmer.py " + sample.label + " " + str(idx) + " " + str(files) + " remote")
    else:
        for i in range(len(files_list)/split+1):
            ...
            print("python diet_tree_skimmer.py " + sample.label + " " + str(i) + " " + ",".join( e for e in files_list[split*i:extmax]) + " remote")
